...
ggplot(data = Citymapper_long_filter_mar2020_eu,
  ...
  theme(strip.text = element_text(size=12,vjust = 1))+
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 15, b = 0, l = 0),vjust = 1.5))

```
This graph indicates the evolution of the mobility index in major European cities based on data collected by "Citymapper". Interesting to note that while mobiltiy in Paris seems to have resumed to (almost) pre-covid, in other cities (e.g. Milan), it is not yet the case.
...
Users are public transport users and also use us for walking, cycling, and some micromobility and cabs (driving is not included). According to the provider, there are enough data points  to be confident that it represents a real change in behaviour. However, it is a sample set of general mobility and may not represent the real world exactly.
